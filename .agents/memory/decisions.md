...
**Not fixed here, and worth its own change:** the 10-minute re-fan itself. The cap converts it from "blanks
the plane" into "spends its own quarter, forever", which is bounded but still mostly redundant. The fix is a
profile dedup keyed on author + `profileVersion` with a TTL nearer the 12-hour republish period than the
SeenSet's 10 minutes.
