 ...
 *
 * **Every entry point is `@Synchronized`, because this queue has three structural writers on three
 * coroutines.** `take` runs on the pacer loop; `enqueue` on whichever coroutine sent (the composite's
 * fan-out, the ctl-packet handler's backfill, the gossip loop's OFFER, the link-state collector's beacon),
 * and it removes a superseded frame before adding; `evictOversize` on the link-state collector. `ArrayDeque`
 * is not thread-safe, and a structural change under `admitBest`'s index walk nulls the slot it is about to
 * read — field-observed on the lab P7 as `NullPointerException: … OutboundFrame.getBucket()` at
