...
## 045. Setting up a board is one step that takes the RF slot with it: Knit as primary, housekeeping quieted
...
*Simplified 2026-08-26, before any of it shipped:* the first cut kept the old secondary-channel write as a
lighter "rendezvous" mode beside the full one, plus a hand-editable channel index. That is three states for
one board and two kinds of Knit fleet that cannot hear each other, all so a board could go on being half a
public Meshtastic node. It is now **one** setup and its undo — a board is configured for Knit or it is a
stock Meshtastic node — which is also what lets the plane assume every Knit board is configured identically.
...
hour than Knit does at chat pace. So "Set up this board for Knit" now does all of it, and can be undone.
...
2. **It is one bargain, so it is one button — and no lighter setting beside it.** The setup write also
   stretches `node_info_broadcast_secs`,
   ...
   what the board broadcasts is Knit's business, powering down the user's hardware is not. Restoring is the
   only other state, and it leaves **no** Knit channel behind — so the plane is switched off with it, rather
   than fanning frames out over whatever channel the board landed back on. Restoring a board that was never
   set up is refused: writing the stock default over somebody's own primary is not an undo of anything. The
   transport backs this with a send-time guard — a bound slot that is not the Knit channel transmits nothing,
   since Knit's frames are cleartext and the channel it would land on is very likely the public one.
...
4. **The intervals the board had are the user's, and are recorded.** A setup returns them
   ...
   when nothing was recorded. Re-running the setup on a board that already has it is a no-op that reports
   `alreadyPresent` — overwriting the record with the quieted values would destroy the only copy of what the
   board looked like before. Forgetting the board forgets the record with it. A board an *older* build wrote
   Knit into a secondary slot on is not "set up": the setup claims the primary and disables that leftover, so
   the upgrade needs no migration beyond one tap.
5. **The cost is stated before the tap, and it is fleet-wide.** A set-up board cannot hear, or be heard by,
   a stock node — that includes third-party repeaters that were extending Knit's range for free — and it
   replaces whatever primary channel the board had. A fleet must be all-or-nothing, since a set-up board and
   a stock one are on different frequencies and simply never meet; that is precisely why there is no middle
   setting to get stranded on. So the one action sits behind a confirmation that says all of it, and is
   reversible from the same screen. The PSK is untouched: this is still a rendezvous, not a confidentiality
   boundary, and the deferred per-deployment private PSK now also buys a *different slot*, since the name
   feeds the hash.
...
closes it; a Knit fleet loses public repeaters as free hops; the board's previous primary channel is replaced
rather than remembered, so restoring returns it to the Meshtastic *default* rather than to a custom channel
it may have had; the six-hour intervals are a judgement, not a measurement; and quieting the *mesh* telemetry interval must not silence the phone-only telemetry ADR 041's
